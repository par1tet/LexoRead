...
const literatureBooksKey = "literature_books"
...
func (r *redisRepository) AddLiteratureBook(ctx context.Context, book *models.Book) error {
	...
	return r.redisClient.RPush(ctx, literatureBooksKey, bookJSON).Err()
}

func (r *redisRepository) GetLiteratureBooks(ctx context.Context) ([]*models.Book, error) {
	topBooksJSON, err := r.redisClient.LRange(ctx, literatureBooksKey, 0, 99).Result()
	if err != nil {
		return nil, err
	}
